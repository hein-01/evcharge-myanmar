...
import {
  ArrowDownRight, ArrowUpRight, BadgeCheck, Calendar, Car, Edit3, Eye, Heart, MessageCircle, Pause, Play, Rocket, Share2, Sparkles, Trash2, TrendingUp,
} from "lucide-react";
import { carListings } from "@/data/cars";
import { sellerListings, type SellerListingStatus } from "@/data/sellerListings";
...
export default function SellerDetailPage() {
  ...
  const listing = sellerListings.find((l) => l.carId === carId);

  useEffect(() => {
    document.title = car ? `Manage listing — ${car.name} | EVCharge Seller` : "Seller listing";
  }, [car]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8 text-center">
        <div>
          ...
          <Link to="/marketplace/sell" className="mt-6 inline-block rounded-full bg-foreground px-5 py-2 text-sm font-semibold text-background">Back to seller hub</Link>
        </div>
      </div>
    );
  }
  ...
  const priceVerdict = Math.abs(pricePct) < 3
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-6 flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-start gap-4">
              <div className="hidden h-20 w-28 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-border md:flex"
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">Listed {listing.listedDaysAgo}d ago</span>
                </div>
                <h1 className="mt-2 font-display text-3xl font-bold md:text-4xl">{car.name}</h1>
                <p className="mt-1 text-sm text-muted-foreground">{car.year} · {car.city} · {car.price.toLocaleString()} MMK</p>
              </div>
            </div>

            <div className="flex flex-wrap gap-2">
              <Link to={`/marketplace/buy/${car.id}`} className="inline-flex h-10 items-center gap-1.5 rounded-full border border-border bg-card px-4 text-xs font-semibold transition hover:border-electric/40">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
                  ...
                  {listing.viewsTrend.map((v, i) => {
                    ...
                    return (
                      <div key={i} className="group flex flex-1 flex-col items-center gap-1.5">
                        ...
                        <div className="w-full rounded-t-md bg-gradient-to-t from-electric/40 to-electric" style={{ height: `${Math.max(h, 4)}%` }} />
              ...
              <section className="rounded-3xl border border-border bg-card p-6" style={{ boxShadow: "var(--shadow-card)" }}>
                ...
                <div className="mt-4 divide-y divide-border">
                  {listing.recentInquiries.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No inquiries yet.</div>
                  ) : (
                    listing.recentInquiries.map((q, i) => (
                      <div key={i} className="flex items-start gap-3 py-4 first:pt-0 last:pb-0">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold text-background"
            ...
            <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-3xl border border-electric/30 bg-gradient-to-br from-electric/10 via-card to-eco/10 p-6">
                ...
                <div className="mt-2 font-display text-lg font-bold">{listing.promoted ? "Currently promoted" : "Boost this listing"}</div>
                <button className="mt-4 inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-full bg-foreground px-4 text-xs font-semibold text-background">
              ...
              <div className="rounded-3xl border border-border bg-card p-6">
                ...
                <div className="mt-4 grid gap-2">
                  {listing.status === "paused" ? <ActionBtn icon={Play}>Reactivate</ActionBtn> : <ActionBtn icon={Pause}>Pause listing</ActionBtn>}
...
function StatCard({ icon: Icon, label, value, delta, up }: { icon: typeof Eye; label: string; value: string; delta?: string; up?: boolean }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-4" style={{ boxShadow: "var(--shadow-card)" }}>
      <div className="flex items-center justify-between">
        <Icon className="h-4 w-4 text-electric" />
        {delta && <span className={`text-[10px] font-semibold ${up ? "text-eco" : "text-muted-foreground"}`}>{delta}</span>}
      </div>
...
function ActionBtn({ icon: Icon, children, danger }: { icon: typeof Pause; children: React.ReactNode; danger?: boolean }) {
  return (
    <button className={`inline-flex h-10 w-full items-center gap-2 rounded-xl border px-3 text-xs font-semibold transition ${danger ? "border-ruby/30 text-ruby hover:bg-ruby/10" : "border-border bg-background hover:border-electric/40"}`}>
