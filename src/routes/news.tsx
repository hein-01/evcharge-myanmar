...
export default function NewsPage() {
  useEffect(() => { document.title = "EV News & Guides — EVCharge Myanmar"; }, []);
  const { articles: dbArticles } = useArticles();
  const articles =
    dbArticles.length > 0
      ? dbArticles.map((a) => ({
          id: a.id,
        ...
        <div className="mx-auto max-w-7xl px-6">
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((c) => (
                <button key={c} onClick={() => setCat(c)}
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${cat === c ? "bg-foreground text-background" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
                  {c}
                </button>
              ))}
            </div>
          ...
          {feature && (
            <Link to={`/news/${feature.id}`}
              ...
              <div className="aspect-[16/12] md:aspect-auto"
                style={{ background: `linear-gradient(135deg, oklch(0.72 0.16 ${feature.hue}) 0%, oklch(0.55 0.20 ${feature.hue + 30}) 100%)` }} />
              <div className="flex flex-col justify-center p-8">
                <span className="self-start rounded-full bg-secondary px-3 py-1 text-[10px] font-semibold uppercase tracking-wider">{feature.category}</span>
                ...
                <div className="mt-6 flex items-center gap-3 text-xs text-muted-foreground">
                  <span>{feature.author}</span><span>·</span><span>{feature.date}</span>
