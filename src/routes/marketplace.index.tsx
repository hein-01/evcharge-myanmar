...
import { Car, ArrowRight, Search, Filter } from "lucide-react";
import { useMemo, useState, useEffect } from "react";
import { carListings } from "@/data/cars";

export default function MarketplaceIndexPage() {
  useEffect(() => { document.title = "EV Marketplace — Buy & sell EVs and accessories in Myanmar"; }, []);
      ...
      <main className="pt-28">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-2 flex items-end justify-between gap-4">
            <div>
              <h1 className="font-display text-4xl font-bold md:text-5xl">Electric cars</h1>
              <p className="mt-2 text-muted-foreground">{carListings.length} EVs from verified sellers across Myanmar</p>
            </div>
          ...
          <div className="sticky top-16 z-30 -mx-6 mb-8 mt-8 border-y border-border bg-background/90 px-6 py-4 backdrop-blur">
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              <div className="flex flex-1 items-center gap-2 rounded-full border border-border bg-card px-4 py-2">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by model…"
