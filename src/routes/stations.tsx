...
import { MapPin, Zap, Filter, Search, Clock, Map as MapIcon, List, Phone, Navigation, Route as RouteIcon, Bookmark } from "lucide-react";
import { useMemo, useState, lazy, Suspense, useEffect } from "react";

const StationsMap = lazy(() => import("@/components/StationsMap").then((m) => ({ default: m.StationsMap })));
...
export default function StationsPage() {
  useEffect(() => { document.title = "Charging stations across Myanmar — EVCharge"; }, []);
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="sticky top-16 z-30 -mx-6 mb-8 border-y border-border bg-background/90 px-6 py-4 backdrop-blur">
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              <div className="flex flex-1 items-center gap-2 rounded-full border border-border bg-card px-4 py-2">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by name or operator…"
              ...
              <div className="flex items-center gap-2 overflow-x-auto">
                ...
                {cities.map((c) => (
                  <button key={c} onClick={() => setCity(c)}
                    className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium transition ${city === c ? "bg-foreground text-background" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
                    {c}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => setOpenNow((v) => !v)}
                  className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition ${openNow ? "bg-eco text-background" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>
                  <Clock className="h-3.5 w-3.5" /> Open now
                </button>
                <input type="time" value={time} onChange={(e) => setTime(e.target.value)} disabled={!openNow}
          ...
          {view === "map" ? (
            <div className="pb-20">
              ...
              <p className="mt-3 text-xs text-muted-foreground">Showing {filtered.length} of {stations.length} stations</p>
            </div>
          ) : (
            <div className="grid gap-4 pb-20 md:grid-cols-2 lg:grid-cols-3">
              {filtered.map((s, i) => {
                ...
                return (
                  <div key={s.id} className="group rounded-2xl border border-border bg-card p-5 transition hover:border-electric/40" style={{ boxShadow: "var(--shadow-card)" }}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase ${open ? "bg-eco/15 text-eco" : "bg-ruby/15 text-ruby"}`}>
                          {open ? "Open now" : "Closed"}
                        </span>
                        <button
                          type="button"
                          aria-label={saved[s.id] ? "Remove from saved" : "Save station"}
                          aria-pressed={!!saved[s.id]}
                          onClick={() => toggleSaved(s.id)}
                          className={`inline-flex h-7 w-7 items-center justify-center rounded-full border border-border bg-secondary transition hover:border-electric/40 hover:text-electric ${saved[s.id] ? "text-electric" : "text-muted-foreground"}`}
                        >
                          <Bookmark className={`h-3.5 w-3.5 ${saved[s.id] ? "fill-current" : ""}`} />
                        </button>
                      </div>
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                      <span className="inline-flex items-center gap-1 font-medium text-foreground">
                        <RouteIcon className="h-3.5 w-3.5 text-electric" />{(0.3 + i * 0.4).toFixed(1)} miles
                      </span>
                      <span className="inline-flex items-center gap-1"><Clock className="h-3.5 w-3.5" />{s.hours}</span>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-2 border-t border-border pt-4 text-xs">
                      ...
                      <div>
                        ...
                        <div className="font-semibold flex items-center gap-1"><MapPin className="h-3 w-3" />{s.available}/{s.total}</div>
                      </div>
                    </div>
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      <a href="tel:+959000000000" className="inline-flex h-9 items-center justify-center gap-1.5 rounded-full border border-border bg-secondary text-xs font-semibold transition hover:border-electric/40 hover:text-electric">
